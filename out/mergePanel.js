...
class MergePanel {
    static createOrShow(context, fileUri, parsed) {
        ...
        const panel = vscode.window.createWebviewPanel('intellijMergeEditor', `Merge: ${path.basename(fileUri.fsPath)}`, column, {
            enableScripts: true,
            localResourceRoots: [
    ...
    constructor(panel, context, fileUri, parsed) {
    ...
    async _applyAndSave() {
        ...
        const i = this._i18n;
        if (unresolved > 0) {
            const answer = await vscode.window.showWarningMessage((0, i18n_1.ti)(i, 'warn.unresolved', { count: unresolved }), (0, i18n_1.ti)(i, 'warn.saveAnyway'), (0, i18n_1.ti)(i, 'warn.cancel'));
            if (answer !== (0, i18n_1.ti)(i, 'warn.saveAnyway')) {
                return;
            }
        }
        const result = (0, conflictParser_1.applyResolutions)(this._parsed);
        fs.writeFileSync(this._fileUri.fsPath, result, 'utf8');
        if (unresolved === 0) {
            // Mark as resolved in git
            try {
                await (0, gitHelper_1.markResolved)(this._fileUri.fsPath);
                vscode.window.showInformationMessage((0, i18n_1.ti)(i, 'info.mergeDone', { file: path.basename(this._fileUri.fsPath) }));
            }
            catch {
                vscode.window.showInformationMessage((0, i18n_1.ti)(i, 'info.savedNoGit'));
            }
        }
        else {
            vscode.window.showInformationMessage((0, i18n_1.ti)(i, 'info.savedWithConflicts', { count: unresolved }));
        }
        // Re-parse the saved file to reflect the new state
        const newContent = fs.readFileSync(this._fileUri.fsPath, 'utf8');
        this._parsed = (0, conflictParser_1.parseConflicts)(newContent);
        this._panel.webview.postMessage({
            type: 'fileUpdated',
            payload: this._buildPayload(),
        });
    ...
    async _enrichLabelsFromGit() {
        try {
            const info = await (0, gitHelper_1.getGitInfo)(this._fileUri.fsPath);
            if (!info) {
                return;
            }
            ...
            if ((this._parsed.theirsLabel === 'MERGE_HEAD' || this._parsed.theirsLabel === 'REMOTE') &&
                info.mergeHead) {
    ...
    _buildPayload() {
        return {
            ...
            sections: this._parsed.sections.map(s => s.kind === 'context'
                ? { kind: 'context', lines: s.lines, fileLineStart: s.fileLineStart }
                : { kind: 'conflict', hunkIndex: s.hunkIndex }),
        };
    }
    _buildHtml() {
        const webview = this._panel.webview;
        const webviewUri = (file) => webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, 'media', 'webview', file));
        const vendorUri = (file) => webview.asWebviewUri(vscode.Uri.joinPath(this._context.extensionUri, 'media', 'vendor', file));
...
//# sourceMappingURL=mergePanel.js.map